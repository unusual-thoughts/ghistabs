 ...
 */
class FixturesTest {
    private val corpus = listOf("bouniaf.exe", "box2d_tests", "crypto_mi_test_gcc345.exe")

    @Test
    fun classNameMatchesTheGeneratedFiles() {
        assertEquals("AppqueryExeConcurrentTest", Fixtures.className("bouniaf.exe", "CONCURRENT"))
    ...
    fun filtersNarrowBothAxesAndTolerateWhitespace() {
        val f = Fixtures(corpus, fixtureFilter = " box2d_tests , bouniaf.exe ", modeFilter = "after")
