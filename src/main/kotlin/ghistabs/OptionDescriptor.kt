 ...
 * [asOption]/[read] are that second boundary.
 */
abstract class OptionDescriptor<T : Any>(val name: String, val desc: String, val default: T) :
    PropertyDelegateProvider<OptionDescriptor.Container, OptionDescriptor<T>.Holder> {
    ...
    inner class Holder {
        ...
        fun get() = value.asOption()
        fun set(edited: Option) {
            value = read(edited)
        }
        fun read(options: Options) {
        ...
        val option get() = this@OptionDescriptor
...
private fun OptionDescriptor<*>.invalid(option: Option) =
    OptionException("Invalid value for option $name: ${option.value}")
operator fun <T : Any> Options.get(o: OptionDescriptor<T>) = o.get(this)
operator fun <T : Any> Options.set(o: OptionDescriptor<T>, value: T) = o.set(this, value)
 ...
 * with the option's default instead. [ghistabs.importer.ImportOptions] takes that bucket once, in its constructor.
 */
operator fun <T : Any> Program.get(o: OptionDescriptor<T>, cat: String = Program.PROGRAM_INFO) = getOptions(cat)[o]

/** Opens its own transaction, and nests inside an open one — the outermost governs the commit. */
operator fun <T : Any> Program.set(o: OptionDescriptor<T>, cat: String = Program.PROGRAM_INFO, value: T) =
    runTransaction("Stabs: set ${o.name} option") {
        getOptions(cat)[o] = value
    }

class BoolOption(name: String, desc: String, default: Boolean) : OptionDescriptor<Boolean>(name, desc, default) {
...
) : OptionDescriptor<T>(name, desc, default) {
 ...
 */
class DirectoryOption(name: String, desc: String) : OptionDescriptor<String>(name, desc, "") {
    ...
    override fun String.asOption() = Opt(this)
...
open class OptionContainer :
    ArrayList<OptionDescriptor<*>.Holder>(),
    OptionDescriptor.Container {
    override fun <T : Any> bind(option: OptionDescriptor<T>): OptionDescriptor<T>.Holder = option.Holder().also {
        this += it
    }

    /** Takes the dialog's edits back. An unknown name is a wiring bug, not user input. */
    fun export() = map { it.get() }
    fun import(options: List<Option>) = options.forEach { edited ->
        firstOrNull { it.option.name == edited.name }?.set(edited)
            ?: throw OptionException("Unknown option: ${edited.name}")
    }

    fun import(options: Options) = this.forEach { it.read(options) }
    fun exportTo(options: Options) = this.forEach { it.write(options) }
}
