...
/**
 * Two-way oracle used by [contentHash]: an id-keyed lookup for [TypeDecl.Ref]
 * resolution and a name+kind lookup so [TypeDecl.XRef] can resolve to the
...
fun TypeDecl<GlobalTypeId>.contentHash(
    ...
    is TypeDecl.Range -> java.util.Objects.hash("Range", of.refKey(oracle, cache, visited), min, max)

    is TypeDecl.Pointer -> java.util.Objects.hash("Pointer", pointee.contentHash(oracle, cache, visited))

    is TypeDecl.Reference -> java.util.Objects.hash("Reference", referent.contentHash(oracle, cache, visited))

    is TypeDecl.Const -> java.util.Objects.hash("Const", inner.contentHash(oracle, cache, visited))

    is TypeDecl.Volatile -> java.util.Objects.hash("Volatile", inner.contentHash(oracle, cache, visited))

    is TypeDecl.Array -> java.util.Objects.hash(
        "Array",
    ...
    is TypeDecl.Struct -> java.util.Objects.hash(
        "Struct",
        kind,
        sizeBytes,
    ...
    is TypeDecl.FunctionT -> java.util.Objects.hash(
        "FunctionT",
        ret.contentHash(oracle, cache, visited),
        params.map { it.contentHash(oracle, cache, visited) },
    )

    is TypeDecl.Method -> java.util.Objects.hash(
        "Method",
    ...
    is TypeDecl.Builtin -> java.util.Objects.hash("Builtin", slot)

    // (kind, tagName) — primitives only
    is TypeDecl.WithSizeAttr -> java.util.Objects.hash(
        "WithSizeAttr",
        sizeBits,
...
private fun GlobalTypeId.refKey(
    ...
    val referenced = oracle.byId(this) ?: return java.util.Objects.hash("unresolved", n)
...
private fun BaseDecl<GlobalTypeId>.contentHash(
    ...
    cache: MutableMap<GlobalTypeId, Int>?,
    visited: Set<GlobalTypeId>,
) = java.util.Objects.hash("Base", type.contentHash(oracle, cache, visited), isVirtual, access, offsetBits)
