...
private class RenderContext(val renderer: Renderer, val source: String) {
    ...
    private fun staleTag(line: Int) = if (line > activityExtent) "${lineTag(line)} stale N_SOL?" else lineTag(line)
    ...
    private fun emitDecl(
        ...
        when {
            parts == null -> canvas[line] += Fragment(indent, "$decl;", tag)
            parts.size == 1 -> canvas[line] += Fragment(indent, "$decl = ${parts[0]};", tag)
            else -> canvas.layoutBraceBlock(line, indent, "$decl = {", tag, parts, "};", ",", ", ")
        }
    }
    ...
    private fun emitParamsAndLocals() {
        ...
        for (f in rawFuncs) {
            ...
            fun suffix(declLine: Int, base: String) =
                if (span != null && declLine in span) base else "$base; stale N_SOL?"
            for (p in f.params) {
                if (p.sourceFile != source) continue
                when (val d = p.body) {
                    is SymbolDecl.StackParam -> emitDecl(p.declLine, d.name, d.type, suffix(p.declLine, "(param)"))
                    is SymbolDecl.RegParam -> emitDecl(p.declLine, d.name, d.type, suffix(p.declLine, "(reg param)"))
                    else -> {}
                }
            }
            for (l in f.locals) {
                ...
                when (val d = l.body) {
                    is SymbolDecl.RegLocal -> emitDecl(l.declLine, d.name, d.type, suffix(l.declLine, "(reg local)"))
                    is SymbolDecl.StackLocal ->
                        emitDecl(l.declLine, d.name, d.type, suffix(l.declLine, "(stack local)"))
                    else -> {}
                }
            }
        }
    ...
    private fun emitFunctionBraces() {
        for (r in spans.ranges) {
            ...
            val opener = if (r.isSingleLine) {
                Fragment(code = "$sig;", comment = "/* $tag — ${r.func.decl.name} */")
            } else {
                Fragment(code = "$sig {", comment = "/* $tag — opens ${r.func.decl.name} */")
            }
            canvas[r.startLine].fragments.add(0, opener)
            ...
            canvas[closeLine] +=
                Fragment(code = "}", comment = "/* $closeTag — closes ${r.func.demangledName(program)} */")
        }
    }
    ...
    private fun emitTypeBodies() {
        ...
        for (ast in typeDecls.filter { it.declLine in 1..maxLine }.sortedBy { it.declLine }) {
            ...
            if (members.isNotEmpty()) {
                canvas.layoutBraceBlock(line, indent, open, staleTag(line), members, "}; $sizeNote", itemSuffix, sep)
            } else {
                val keyword = if (body is TypeDecl.Struct) body.kind.cxxKeyword() else "enum"
                canvas[line] += Fragment(indent, "$keyword $name; $sizeNote", staleTag(line))
            }
        }
    }

    /**
     * Replace each function's span with its decompilation. Delimiters, SLINE annotations
     * and param/local decls in the span are dropped; other content (type decls, globals)
     * is kept as tail after the close brace. Overflow crams onto the last line.
     */
    private fun applyDecompilation(decomp: DecompInterface) {
        for (b in canvas.multiFragmentLines()) {
            b.fragments.removeAll { it.comment?.contains("stale N_SOL?") == true }
        }
        for (r in spans.ranges) {
            val closeLine = spans.closeLine(r.func) ?: continue
            ...
            val tail = mutableListOf<Fragment>()
            for (line in r.startLine..closeLine) {
                canvas[line].fragments.removeAll { f ->
                    val trivial = f.isFuncDelim ||
                        f.isSlineAnnotation ||
                        f.comment?.let(PARAM_OR_LOCAL::matches) == true
                    if (!trivial) tail += f
                    true
                }
            }
            ...
            if (cLines.size <= available) {
                cLines.forEachIndexed { i, l -> canvas[r.startLine + i] += Fragment(code = l) }
            } else {
                for (i in 0 until available - 1) canvas[r.startLine + i] += Fragment(code = cLines[i])
                val rest = cLines.subList(available - 1, cLines.size)
                    .map { it.trim().trimEnd(';') }
                    .filter { it.isNotEmpty() }
                canvas[closeLine] += Fragment(code = rest.joinToString("; ") + ";")
            }
            tail.forEach { canvas[closeLine] += it }
        }
    }
