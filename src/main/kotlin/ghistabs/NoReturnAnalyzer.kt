 ...
 *
 * unbouniaf's `error()` calls `exit` and never comes back, but nothing marks it, so every caller
 ...
 * whatever the linker placed next, which is no part of this function. unbouniaf's `error` ends at
