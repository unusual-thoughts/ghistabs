...
class BlockScopesTest {
    ...
    private fun BlockTreeBuilder.local(name: String, declLine: Int) = local(
        Symbol(
            ...
            sourceFile = sourceFileOf("unpackfile.cpp"),
        ),
    )
    ...
    fun `a block owns the symbols emitted before its LBRAC, not the ones between its brackets`() {
        val (_, blocks) = mainBuilder().finish(lines, sourceFileOf("unpackfile.cpp"))
    ...
    fun `a local's source is its block's, not the N_SOL left over at the closing brace`() {
        val (locals, _) = mainBuilder().finish(lines, sourceFileOf("unpackfile.cpp"))
    ...
    fun `a block whose own code spans several files falls back to the enclosing block`() {
        ...
        val (locals, _) = mainBuilder().finish(spanning, sourceFileOf("unpackfile.cpp"))

        assertEquals("stl_alloc.h", locals.first { it.line == 664 }.sourceFile.filename)
        assertEquals("unpackfile.cpp", locals.first { it.body.name == "fs" }.sourceFile.filename)
    }
    ...
    fun `a local no block claims belongs to the function`() {
        val (locals, blocks) = mainBuilder().apply { local("orphan", 27) }.finish(lines, sourceFileOf("unpackfile.cpp"))

        assertEquals("unpackfile.cpp", locals.single { it.body.name == "orphan" }.sourceFile.filename)
