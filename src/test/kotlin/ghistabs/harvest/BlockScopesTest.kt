...
class BlockScopesTest {
    ...
    fun `a local's source is its block's, not the N_SOL left over at the closing brace`() {
        val sources = buildBlocks(brackets, locals).attributedSources(lines, "unpackfile.cpp")

        assertEquals(
            mapOf(
                1771 to "unpackfile.cpp", // the function's own local: inherits the function
                1773 to "stl_alloc.h",
    ...
    fun `a block whose own code spans several files falls back to the enclosing block`() {
        ...
        val sources = buildBlocks(brackets, locals).attributedSources(spanning, "unpackfile.cpp")
        ...
        assertEquals("unpackfile.cpp", sources[1771])
    }
}
