...
class SourceFoldingTest {
    ...
    fun sameHeaderUnderDifferentBuildRootsFolds() {
        ...
        for (i in inputs) assertEquals("/jenkins/bluesuite/result/include/xvimage/image.h", map.getValue(i))
    }
    ...
    fun equalDepthRootsPickTheSameOneEveryTime() {
        ...
        val inputs = listOf("/work/include/xvimage/image.h", "/jenkins/include/xvimage/image.h")
        val map = foldSourcePaths(inputs)
        val reversed = foldSourcePaths(inputs.reversed())
        for (i in inputs) assertEquals("/jenkins/include/xvimage/image.h", map.getValue(i))
