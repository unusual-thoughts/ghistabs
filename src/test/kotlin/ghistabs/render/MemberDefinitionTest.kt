...
class MemberDefinitionTest {
    ...
    fun `a qualified constructor loses its return type and its explicit this`() {
        assertEquals(
            "bouniaf::bouniaf() { int x; }",
            "void bouniaf::bouniaf(bouniaf *this) { int x; }".asMemberDefinition(),
        )
        assertEquals("bouniaf::~bouniaf()", "void bouniaf::~bouniaf(bouniaf *this)".asMemberDefinition())
    }

    @Test
    fun `an ordinary member keeps its return type and loses only this`() {
        assertEquals(
            "int bouniaf::ConvAddrFromFile(int addrIn)",
            "int bouniaf::ConvAddrFromFile(bouniaf *this,int addrIn)".asMemberDefinition(),
        )
    }
    ...
    fun `an array extent follows the declarator, as C requires`() {
        assertEquals("char const _ZTS7MyKlass[9]", declarator("char const[9]", "_ZTS7MyKlass"))
