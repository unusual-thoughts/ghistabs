 ...
 */
fun <T> ParseResult<T>.expectOk(): T = when (this) {
    is ParseResult.Ok -> inner
    is ParseResult.Error -> fail("expected a parse, got: ${ex.message}")
}

/** The parse failure, for tests asserting the parser rejects input rather than misreading it. */
fun <T> ParseResult<T>.expectError(): StabsParseException = when (this) {
