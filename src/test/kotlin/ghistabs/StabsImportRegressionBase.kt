...
abstract class StabsImportRegressionBase(val binaryName: String, val mode: Mode) :
    ...
    fun cSymLexStreamVtableAddressPointSkipsVbaseOffset() {
        assumeTrue(binaryName == "xapasmcsr.exe", "Skipping: CSymLexStream specific to xapasmcsr.exe")
        // CSymLexStream derives (via CLexStream → basic_ifstream) from basic_istream, which
        // virtually inherits basic_ios, so _ZTV13CSymLexStream is preceded by a vbase-offset word:
        ...
        val ztv = program.symbolTable.getSymbols("__ZTV13CSymLexStream").firstOrNull()
        assumeTrue(ztv != null, "Skipping: CSymLexStream vtable not resolved")
        ...
        Assertions.assertTrue(
            ...
            "no vftable symbol at CSymLexStream address point $addressPoint (ztv+3*ptr); " +
    ...
    fun typeInfoBaseClassesNotLeftAsStubs() {
        assumeTrue(binaryName == "xapasmcsr.exe", "Skipping: libsupc++ RTTI base classes specific to xapasmcsr.exe")
    ...
    @ExpectedToFail(
        fixtures = [
            "appquery.exe", "packfile.exe", "xapasmcsr.exe", "xmltest",
     ...
     * the materialize-winner loop regardless of order: CSymLexStream's
