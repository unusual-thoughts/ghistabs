...
class SourceRootTest {
    ...
    fun `deeper agreement on the path wins first`() {
        tree("proj/util/xvimage/image.h", "vendor/xvimage/image.h")
        val derived = derive("/E:/work/cc/util/xvimage/", "image.h")

        assertEquals("$root/proj/util/xvimage/", derived.transforms.single().local)
    }
