...
class BuiltinTableTest {
    ...
    @Test fun testUnsignedInt32() {
        val r = BuiltinTable.resolve(TypeDecl.Range(TypeId(0, 1), 0L, 4294967295L), dtm)!!
        assertEquals(4, r.length)
        assertTrue(r is AbstractUnsignedIntegerDataType || r.name.contains("uint", ignoreCase = true) || r is UnsignedIntegerDataType)
    }

    @Test fun testUnsignedByte() {
        val r = BuiltinTable.resolve(TypeDecl.Range(TypeId(0, 1), 0L, 255L), dtm)!!
        assertEquals(1, r.length)
    }

    @Test fun testWithSizeAttr64ULL() {
