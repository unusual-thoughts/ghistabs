                ...
                out.bufferedWriter().use { w ->
                    ...
                    for (n in tracked.sorted().distinct()) {
                        ...
                        if (entry != null) {
                            ...
                            w.write(
                                "  ,$n → ${b::class.simpleName} name=${entry.value.name} body=$b\n".take(360) + "\n",
                            )
                        } else {
                            w.write("  ,$n → MISSING\n")
                        }
                    ...
                    for ((cu, ns) in perSoIds) {
                        w.write(
                            "  $cu: ${ns.size} N_LSYM bindings (0,N) — sample: ${
                                ns.sorted().take(40).joinToString(",")
                            }\n",
                        )
                    }
                    ...
                    for (rec in reader.records) {
                        ...
                        if (curSo != "/xml/tinyxml2.cpp") continue
                        if (rec.type.name != "N_LSYM" &&
                            rec.type.name != "N_GSYM" &&
                            rec.type.name != "N_PSYM"
                        ) {
                            continue
                        }
