...
import ghistabs.entrypoints.StabsAnalyzer.Companion.disableStabsAnalyzer
...
const val OUTPUT_ROOT = "build/test-output"
...
abstract class StabsImportRegressionBase(val binaryName: String, val mode: Mode) :
    ...
    private val shiftedBaselineFile get() = File(
        "$OUTPUT_ROOT/shifted-baselines/${fixture.nameWithoutExtension}-baseline.json",
    )
    private val recordsFile get() = outputFile("record")
    private val emptyStubDumpFile get() = File("$OUTPUT_ROOT/demangler-empty-stubs/${fixture.nameWithoutExtension}.txt")
    ...
    private val degradationFile get() = File(
        "$OUTPUT_ROOT/degradations/${fixture.nameWithoutExtension}.${mode.name.lowercase()}.txt",
    )
    private val logFile get() = File("$OUTPUT_ROOT/logs/${fixture.nameWithoutExtension}.${mode.name.lowercase()}.log")
    private val analysisTimesFile get() = File(
        ...
        try {
            ...
            when (mode) {
                Mode.CONCURRENT -> {
                    ...
                    program.enableStabsAnalyzer(context.options)
    ...
    /**
     * Only a `_ZTV` group's primary bears a symbol, so what marks a secondary inside the object is its
     * rtti header — a word holding the address of a `_ZTI…` symbol. Every one must be followed by an
     * `internal_vftable`; an unlabelled one is an address point some subobject's vfptr holds that the
     * decompiler resolves to a raw address (`_ZTVSi+32`, the §54 case).
     *
     * Independent of the walk under test, which bounds itself by rtti *value* equality: this reads the
     * symbol table and stops at the next Itanium-mangled symbol.
     */
    @Test
