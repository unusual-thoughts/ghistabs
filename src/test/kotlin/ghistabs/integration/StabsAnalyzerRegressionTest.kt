...
abstract class StabsAnalyzerRegressionTest(private val mode: Mode) : AbstractGhidraHeadlessIntegrationTest() {
    ...
    fun dcinstVtableMatchesItaniumLayout() {
        ...
        // The function pointers live inside bouniaf_vmethods (what {vfptr} actually points to).
        val vmethods = program.dataTypeManager.allDataTypes
            .asSequence()
            .filterIsInstance<Structure>()
            .filter { it.name == "bouniaf_vmethods" }
            .maxByOrNull { it.numComponents }
        Assertions.assertNotNull(vmethods, "bouniaf_vmethods not found")
        ...
        Assertions.assertTrue(
            missing.isEmpty(),
            "bouniaf_vmethods missing virtuals: $missing (have: $virtuals)",
        )
    }
