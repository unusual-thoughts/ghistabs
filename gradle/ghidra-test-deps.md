...
- Individual integration tests guard with `assumeTrue(fixture.exists())`, so tests skip gracefully if `xapasmcsr.exe` fixture is absent.
...
- Tests will skip via `assumeTrue(fixture.exists())` regardless, since `xapasmcsr.exe` is EULA-restricted and not committed to the repo.
