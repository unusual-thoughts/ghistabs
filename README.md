...
[![Ghidra 11.4.3](https://img.shields.io/badge/Ghidra-11.4.3-orange?logo=redragon)](https://github.com/unusual-thoughts/ghistabs/releases)
[![Ghidra 10.4](https://img.shields.io/badge/Ghidra-10.4-red?logo=redragon)](https://github.com/unusual-thoughts/ghistabs/releases)

A toolkit for [**STABS**](https://sourceware.org/gdb/onlinedocs/stabs.pdf) debug info, built on
Ghidra, which ships no stabs support of its own. Load a binary carrying stabs (in `.stab` / `.stabstr` sections or in the symbol table) and the
**Stabs Importer** analyzer picks it up during auto-analysis.

A staged pipeline decodes the raw records, parses each declaration, and joins the per-CU fragments into one program-wide model. Everything the compiler recorded lands in the program: data types, function signatures, parameters and locals, C++ classes and vtables, constants, source maps, and more. A further pass can export the original source files back out.
...
as well as ELF targets until they moved to DWARF-2 in gcc **3.1** (2002), while Cygwin and MinGW
targets kept it until **4.3.0** (2008). So stabs found in the wild are often old MinGW .exe's,
whose records Ghidra leaves sitting in the section unparsed.

Stabs was never standardized, each UNIX vendor having their own dialect, and gcc itself emitted it
differently across two decades of releases, so there is quite a bit of interpreting to do.
`ghistabs` parses the grammar in full and resolves the fragments into one whole-program type
graph across compilation units - then reconciles that against what Ghidra independently worked out
about the binary: demangled names, discovered layouts, symbols, calling conventions, decompiled
bodies.
...
the parsed symbol declarations from each `name:descriptor` string, the whole-program type graph
those merge into, and the Ghidra data types, functions, etc it materializes. `ghistabs` also runs
as a **standalone headless CLI**, requiring only a Ghidra install and no GUI or project workflow.

It can also **recreate quasi-original C/C++ source code**: one file per original source file with
line-aligned `#include`s, typedefs and full struct/union/enum/class bodies, globals and statics
under their declared types and with their initializers, and decompiled function bodies whose
signatures, parameter names, local names and types are the ones the compiler recorded.
...
Runs automatically during auto-analysis on any program carrying stabs - `.stab` / `.stabstr`
blocks on PE and ELF, `.symtab` / `.strtab` on a.out - at `LOW_PRIORITY`, after Ghidra's
demangler, whose output it builds on. It imports **once per program**: a `Stabs Imported` flag is stored in the program info, so
...
questions. Requires the importer to have run first.

- **Stabs Source Skeleton** (`.skeleton`) contains everything the debug info places in each file,
  at its original line - typedefs, type bodies, globals, function signatures, every parameter and
...
skeleton when you want to see what the stabs *claimed*; read `decomp` when you want the code.
...
- [uyjulian/stab_debuginfo_utils](https://github.com/uyjulian/stab_debuginfo_utils) - STAB
  parsing and gdb export scripts
