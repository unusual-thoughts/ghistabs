...
- **`-Pfixture=<filename>[,<filename>…]`** — narrow every parameterised suite *at the source* (via
  ...
  exact filenames, **extension included**: `xapasmcsr.exe`, `xmltest`, `appquery.exe`, `box2d_tests`,
  `packfile.exe`, `unpackfile.exe` (+ the extended gcc-4.2.1/3.4.5 corpus). A filter that selects
  nothing from a suite errors loudly.
  ```
  ./gradlew integrationTest -Pfixture=xapasmcsr.exe,box2d_tests
  ```
